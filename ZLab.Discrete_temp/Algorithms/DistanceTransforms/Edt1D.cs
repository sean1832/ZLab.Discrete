...
namespace ZLab.Discrete.Algorithms.DistanceTransforms
{
    ...
    internal static class Edt1D
    {
        ...
        public static void Transform_Isotropic(ReadOnlySpan<int> input, Span<int> output)
        {
            ...
            if (length == 0) return;
            if (input.Length != output.Length) throw new ArgumentException("input and output must have the same length");

            int[]? rentedCandidates = null;
            double[]? rentedBounds = null;

            Span<int> candidatePositions =
                length <= StackThreshold
                    ? stackalloc int[length] // stackalloc for small arrays
                    : (rentedCandidates = ArrayPool<int>.Shared.Rent(length)).AsSpan(0, length);

            Span<double> regionBounds =
                (length + 1) <= StackThreshold
                    ? stackalloc double[length + 1] // stackalloc for small arrays
                    : (rentedBounds = ArrayPool<double>.Shared.Rent(length + 1)).AsSpan(0, length + 1);
            ...
            {
                if (rentedCandidates is not null) ArrayPool<int>.Shared.Return(rentedCandidates);
                if (rentedBounds is not null) ArrayPool<double>.Shared.Return(rentedBounds);
            }
